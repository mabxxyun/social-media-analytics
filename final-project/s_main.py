...
if __name__ == "__main__":
    ...
    if args.resource:
        ...
        Base +=['resource_allocation_index']
    ...
    if args.randomforest:
        ...
        if args.visualize:
            Confusion_Matrix(ytrain, y_train_pred, 'RF', 'Train', args.dataset)
            Confusion_Matrix(ytest, y_test_pred, 'RF', 'Test', args.dataset)
            ROC_Curve(ytrain, y_train_pred, 'RF', 'Train', args.dataset)
            ROC_Curve(ytest, y_test_pred, 'RF', 'Test', args.dataset)
            Importance(RDForest, Base, 'RF', args.dataset)
    ...
    if args.gradientboost:
        ...
        if args.visualize:
            Confusion_Matrix(ytrain, y_train_pred, 'GB', 'Train', args.dataset)
            Confusion_Matrix(ytest, y_test_pred, 'GB', 'Test', args.dataset)
            ROC_Curve(ytrain, y_train_pred, 'GB', 'Train', args.dataset)
            ROC_Curve(ytest, y_test_pred, 'GB', 'Test', args.dataset)
            Importance(clf, Base, 'GB', args.dataset)
